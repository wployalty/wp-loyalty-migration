        ...
        <div class="wlrmg-export-content">
            ...
            <div>
                <form method="post" class="wlrmg-export-preview" id="wlrmg-export-preview" enctype="multipart/form-data"
                      action="<?php echo ! empty( $base_url ) ? esc_url( $base_url ) : ""; ?>"
                    ...
                    <input type="hidden" name="category"
                           value="<?php echo ! empty( $category ) ? esc_html( $category ) : ''; ?>">
                    <input type="hidden" id="wlrmg_limit_start" name="limit_start"
                           value="<?php echo ! empty( $limit_start ) ? esc_html( $limit_start ) : 0; ?>"/>
                    <input type="hidden" id="job_id" name="job_id"
                           value="<?php echo ! empty( $job_id ) ? esc_html( $job_id ) : 0; ?>"/>
                    <input type="hidden" name="wlrmg_nonce"
                           value="<?php echo ! empty( $wlrmg_nonce ) ? esc_html( $wlrmg_nonce ) : " "; ?>"/>
                    <input type="hidden" id="limit" name="limit"
                           value="<?php echo ! empty( $limit ) ? esc_html( $limit ) : 5; ?>"/>
                    <input type="hidden" id="wlrmg-total-count" name="total_count"
                           value="<?php echo ! empty( $total_count ) ? esc_html( $total_count ) : 0; ?>"/>
                    <div class="wlrmg-export-label">
                        <div>
                            <p><?php esc_html_e( "Total items", "wp-loyalty-migration" ); ?></p>
                            <p><?php echo ! empty( $total_count ) ? esc_html( $total_count ) : 0; ?></p>
                        </div>
                        <div>
                            <p><?php esc_html_e( "Processed items", "wp-loyalty-migration" ); ?></p>
                            <p id="wlrmg-process-count"><?php echo ! empty( $process_count ) ? esc_html( $process_count ) : 0; ?></p>
                        </div>
                    </div>
                </form>
            </div>
