                    ...
                    <?php if (isset($condition_status) && !empty($condition_status) && isset($condition) && !empty($condition)): ?>
                        <?php foreach ($condition_status as $key => $status): ?>
                            <div class="wlrmg-filter-status">
                                <a href="<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLRMG_PLUGIN_SLUG, "view" => "activity", "condition" => $key))); ?>" <?php echo $key === $condition ? 'class="active-filter"' : "" ?>><?php echo __($status, "wp-loyalty-migration") ?></a>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
                        ...
                        <div class="actions wlba-grid-span-2">
                            <div>
                                <a class="view_details"
                                   ...
                                   href="<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLRMG_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>">
