        ...
        <form action="<?php echo isset($base_url) ? $base_url : ""; ?>" method="post"
            ...
            <div class="wlrmg-activity-header">
                ...
                <div class="wlrmg-activity-heading">
                    <h3><?php _e("ACTIVITIES", "wp-loyalty-bulk-action"); ?></h3>
                </div>
                    ...
                    <?php if (isset($condition_status) && !empty($condition_status) && isset($condition) && !empty($condition)): ?>
                        <?php foreach ($condition_status as $key => $status): ?>
                            <div class="wlrmg-filter-status">
                                <a href="<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity", "condition" => $key))); ?>" <?php echo $key === $condition ? 'class="active-filter"' : "" ?>><?php echo __($status, "wp-loyalty-bulk-action") ?></a>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            ...
            <div class="wlrmg-no-activity-yet">
                <div>
                    <img
                        ...
                        alt="<?php echo __("Filter", "wp-loyalty-bulk-action") ?>">
                </div>
                <div>
                    <p style="font-size: 20px; color: #161F31;"><?php echo __("No activities yet", "wp-loyalty-bulk-action") ?></p>
                </div>
                <div>
                    <p style="font-size: 16px; color: #535863;"><?php echo __("Get started by adding points or import customers from wordpress", "wp-loyalty-bulk-action") ?></p>
                </div>
            </div>
        <?php else: ?>
            <div class="wlrmg-activity-list-header">
                <div></div>
                <div class="wlrmg-activity-header "
                ><?php echo __("JOB ID", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header wlba-grid-span-2"
                ><?php echo __("ACTIVITY", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header wlba-grid-span-2"
                ><?php echo __("CATEGORY", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header"
                ><?php echo __("PROCESSED", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header "
                ><?php echo __("STATUS", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header"
                ><?php echo __("REVERT PROCESSED", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header"
                ><?php echo __("REVERT STATUS", "wp-loyalty-bulk-action") ?></div>
                <div class="wlrmg-activity-header wlba-grid-span-2"
                ><?php echo __("ACTION", "wp-loyalty-bulk-action") ?></div>
            </div>
                    ...
                    <div class="wlrmg-activity-list-row">
                        <div class="wlrmg-activity-row-data"
                            ...
                            <img
                                ...
                                alt="<?php echo __("Order point", "wp-loyalty-bulk-action") ?>">
                        </div>
                        <div class="wlrmg-activity-row-data "
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                        ...
                        <div class="wlrmg-activity-row-data wlba-grid-span-2"
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                            <?php echo sprintf(__("%s %s updated", "wp-loyalty-bulk-action"), $activity["points"], $campaign->getPointLabel($activity["points"])) ?>
                        </div>
                        <div class="wlrmg-activity-row-data wlba-grid-span-2"
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                            <?php echo __($activity["action"], "wp-loyalty-bulk-action"); ?>
                        </div>
                        <div class="wlrmg-activity-row-data"
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                            <?php echo __($activity["processed_count"], "wp-loyalty-bulk-action"); ?>
                        </div>
                        <div class="wlrmg-activity-row-data "
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                            <?php echo __($activity["status"], "wp-loyalty-bulk-action"); ?>
                        </div>
                        <div class="wlrmg-activity-row-data"
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                            <?php echo __($activity["revert_processed_count"], "wp-loyalty-bulk-action"); ?>
                        </div>
                        <div class="wlrmg-activity-row-data"
                             onclick="wlrmg.redirectToUrl('<?php echo admin_url("admin.php?" . http_build_query(array("page" => WLBA_PLUGIN_SLUG, "view" => "activity_details", "job_id" => $activity["job_id"]))) ?>')">
                            <?php echo __($activity["revert_status"], "wp-loyalty-bulk-action"); ?>
                        </div>
                        <div class="actions wlba-grid-span-2">
                            <div>
                                <a class="view_details"
                                   title="<?php echo __("View Details", "wp-loyalty-bulk-action") ?>"
                            ...
                            <?php if (isset($activity['show_edit_button']) && $activity['show_edit_button']): ?>
                                <div>
                                    <a class="view_details" title="<?php echo __("Edit", "wp-loyalty-bulk-action") ?>"
                            ...
                            <?php if (isset($activity['show_revert_button']) && $activity['show_revert_button']): ?>
                                <div>
                                    <i title="<?php echo __("Revert", "wp-loyalty-bulk-action") ?>"
