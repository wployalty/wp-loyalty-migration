...
$current_page    = ! empty( $current_page ) ? $current_page : $current_page = "settings";
$option_settings = ! empty( $option_settings ) && is_array( $option_settings ) ? $option_settings : [];
?>
        ...
        <div class="headings">
            ...
            <div class="heading-buttons">
                <button type="button" class="wlrmg-button-action non-colored-button"
                        onclick="wlrmg.redirectToUrl('<?php echo ! empty( $back_to_apps_url ) ? esc_url( $back_to_apps_url ) : '#'; ?>');">
                    <img src="<?php
					// phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
					echo ! empty( $previous ) ? esc_url( $previous ) : ""; ?>"
    ...
    <div class="wlrmg-body-data">
        <form action="" method="post" id="settings-form">
            <div>
				...
				<?php if ( ! empty( $batch_limit ) && is_array( $batch_limit ) ):
                        ...
                        <select name="batch_limit" id="batch-limit"
							...
							<?php foreach ( $batch_limit as $batch_key => $batch_value ): ?>
                                <option value="<?php echo esc_attr( $batch_key ); ?>"
									...
									echo esc_html__( $batch_value, "wp-loyalty-migration" ); ?>
                                </option>
							<?php endforeach; ?>
                        </select>
                    </div>
            ...
            <div>
				...
				<?php
				if ( ! empty( $pagination_limit ) && is_array( $pagination_limit ) ):
                        ...
                        <select name="pagination_limit" id="pagination_limit"
							...
							<?php foreach ( $pagination_limit as $pagination_key => $pagination_value ): ?>
                                <option value="<?php echo esc_attr( $pagination_key ); ?>"<?php echo ( $pagination_key == $selected_pagination ) ? esc_attr( "selected" ) : ""; ?>>
									<?php
									//phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
									echo esc_html__( $pagination_value, "wp-loyalty-migration" ) ?>
                                </option>
							<?php endforeach; ?>
                        </select>
                    </div>
