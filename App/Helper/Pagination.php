...
class Pagination
{
    ...
    function createLinks($pagination_args = array())
    {
        ...
        // If there is only one page, return the count information
        if ($numPages == 1) {
            if ($this->showCount) {
        ...
        // Here, we adjust the query string to add `type=woocommerce_migration`
        $query_string_sep_data = (isset($pagination_args['page_number_name']) && !empty($pagination_args['page_number_name'])) ? $pagination_args['page_number_name'] . '=' : 'migration_page=';

        // Determine the base URL and add additional parameters
        $query_string_sep = '';
        $input = new Input();
        ...
        if (isset($get_job_id) && !empty($get_job_id)) {
            $query_string_sep .= '?job_id=' . $get_job_id;
        }
        $query_string_sep .= '&type=' . $get_type; // Always include `type=woocommerce_migration`

        // Ensure that the `migration_page` is correctly added to the URL
        ...
        if ($this->currentPage > $this->numLinks) {
            ...
            $output .= $this->firstTagOpen . '<a href="' . $this->baseURL . '1' . $focusId . '">' . $this->firstLink . '</a>' . $this->firstTagClose;
        }
        ...
        if ($this->currentPage != 1) {
            ...
            $output .= $this->prevTagOpen . '<a href="' . $this->baseURL . $i . $focusId . '">' . $this->prevLink . '</a>' . $this->prevTagClose;
        }
        ...
        for ($loop = $start - 1; $loop <= $end; $loop++) {
            ...
            if ($i >= 1) {
                if ($this->currentPage == $loop) {
                    ...
                    $output .= $this->numTagOpen . '<a href="' . $this->baseURL . $i . $focusId . '">' . $loop . '</a>' . $this->numTagClose;
                }
            }
        }
        ...
        if ($this->currentPage < $numPages) {
            ...
            $output .= $this->nextTagOpen . '<a href="' . $this->baseURL . $i . $focusId . '">' . $this->nextLink . '</a>' . $this->nextTagClose;
        }
        // Render the "Last" link
        if (($this->currentPage + $this->numLinks) < $numPages) {
            $i = $numPages;
            $output .= $this->lastTagOpen . '<a href="' . $this->baseURL . $i . $focusId . '">' . $this->lastLink . '</a>' . $this->lastTagClose;
        }
