...
class WooPointsRewards implements Base
{
	...
	function migrateToLoyalty($job_data)
	{
		...
		$batch_size = (int)($job_data->batch_size ?? 100);  // Define batch size here
        ...
        LIMIT $batch_size";  // Fetch users in batches
		...
		$this->processUserMigration($wp_users, $job_id, $job_data, $admin_mail, $action_type, $batch_size);
	}
	 ...
	 * @return void
	 */
	private function processUserMigration($wp_users, $job_id, $data, $admin_mail, $action_type, $batch_size)
	{
		$migration_job_model = new ScheduledJobs();
		...
		foreach ($wp_users as $wp_user) {
			...
			$trans_type = 'credit';
			$migration_status = $campaign->addExtraPointAction($action_type, $total_points, $action_data, $trans_type , true , true , true);
