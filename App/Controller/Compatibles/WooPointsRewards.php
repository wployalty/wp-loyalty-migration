	...
	public function migrateUsers( $wp_users, $job_id, $data, $admin_mail, $action_type ) {
		...
		foreach ( $wp_users as $wp_user ) {
			...
			$action_data                = array(
				...
				"note"                => sprintf( __( "%s customer migrated from Woo points and rewards with %d %s by administrator(%s) via WPLoyalty migration", "wp-loyalty-migration" ), $user_email, $new_points, $campaign->getPointLabel( $new_points ), $admin_mail ),
			);
			$trans_type                 = 'credit';
